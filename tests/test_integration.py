...
class Integration(unittest.TestCase):
 def setUp(self):
  ...
  (self.project/'omarchy-plugin/manifest.json').write_text(json.dumps({'entryPoints':{'service':'Service.qml','barWidget':'BarWidget.qml'}}))
  ...
  for p in [patch.object(integration,'ROOT',self.project),patch.object(integration,'BIN_DIR',self.bin),patch.dict('os.environ',{'XDG_CONFIG_HOME':str(self.config),'XDG_DATA_HOME':str(self.root/'data')}),
 ...
 def test_install_is_idempotent_and_preserves_other_settings(self):
  ...
  self.assertEqual(cfg['plugins'],[{'id':'other.plugin'},{'id':'jev-voice'}])
  self.assertEqual(cfg['bar']['layout']['right'],[{'id':'before'},{'id':'jev-voice'},{'id':'after'}])
 ...
 def test_all_qml_files_share_new_revision_when_a_dependency_changes(self):
  self.apply(); dest=self.config/'omarchy/plugins/jev-voice'
